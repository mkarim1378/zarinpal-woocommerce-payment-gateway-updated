            ...
            public function Return_from_ZarinPal_Gateway() {
                ...
                if (isset($_GET['Status']) && $_GET['Status'] === 'OK') {
                    ...
                    try {
                        ...
                        // verify ممکن است به دلیل timeout یا خطای موقت شبکه شکست بخورد
                        // در حالی که بانک پرداخت را با موفقیت انجام داده. قبل از اعلام
                        // شکست، با inquiry وضعیت واقعی تراکنش را از زرین‌پال می‌گیریم.
                        $inquiry_status = null;
                        try {
                            $inquiry = $this->zarinpal->inquiryPayment($authority);
                            $inquiry_status = $inquiry['status'] ?? null;
                        } catch (Exception $e2) {
                            // inquiry هم ناموفق بود؛ کاری از دست ما برنمی‌آید
                        }

                        if (in_array($inquiry_status, array('PAID', 'VERIFIED'), true)) {
                            $order->update_status('on-hold', sprintf(
                                __('پرداخت توسط بانک با وضعیت "%1$s" گزارش شد اما تایید خودکار (verify) با خطا مواجه شد: %2$s. لطفاً از طریق دکمه «بررسی دستی تراکنش» وضعیت را تایید کنید.', WC_ZPAL_TEXT_DOMAIN),
                                $inquiry_status,
                                $e->getMessage()
                            ));
                            wc_add_notice(__('پرداخت شما در حال بررسی است و سفارش شما با موفقیت ثبت شد. نتیجه نهایی به‌زودی اعلام می‌شود.', WC_ZPAL_TEXT_DOMAIN), 'success');
                            wp_redirect($this->get_return_url($order));
                            exit;
                        }
